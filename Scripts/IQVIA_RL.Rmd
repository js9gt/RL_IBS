...
Working to bundle visits 
```{r}
## only for non hospitalizations (no conf_num)
## Then, group any time period with more than 3 days (4 or more days) apart as separate visits

#all dates that are 3 days or less apart are put into one group

library(data.table)
### Subset by non-hosp data (outpatient visits)
outpatient <- ibs_dat %>% filter(is.na(conf_num)) %>%
  #Sort by date
  arrange(from_dt)

outpatient$visit_group <- cumsum(ifelse(difftime(outpatient$from_dt,
                                  shift(outpatient$from_dt, fill = outpatient$from_dt[1]), 
                                  units = "days") >= 3 
                         ,1, 0)) + 1
...
###### run this after running all the read_data steps to have access to the drug groupings
...
outpatient <- outpatient %>% 
  ...
  mutate(between_dt =as.numeric(c(diff(from_dt),0))/86400) %>% select(from_dt, visit_group, between_dt)

# Calculating
subs <- outpatient %>% distinct(from_dt, visit_group, between_dt) %>% select(from_dt, visit_group, between_dt) %>% View()

sum(subs$between_dt > 3) / length(unique(subs$visit_group))
